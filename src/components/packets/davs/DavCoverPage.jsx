...
export const DavCoverPage = ({ blankMode = false }) => {
  return (
    <div className="relative bg-white text-slate-900 font-sans shadow-2xl mx-auto border border-slate-300 p-12 space-y-8" style={{ width: '850px', height: '1100px' }}>
      ...
      <div className="bg-slate-100 p-6 rounded-xl border border-slate-300 space-y-4 font-mono text-xs">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>PATIENT: {blankMode ? <span className="border-b border-slate-400 inline-block w-32">&nbsp;</span> : <strong>SAMPLE TESTING</strong>}</div>
          <div>SYSTEM ID: {blankMode ? <span className="border-b border-slate-400 inline-block w-24">&nbsp;</span> : <strong>141849159</strong>}</div>
          <div>ACCIDENT DATE: {blankMode ? <span className="border-b border-slate-400 inline-block w-24">&nbsp;</span> : <strong>12/27/2025</strong>}</div>
          <div>TOTAL BILL BALANCE: {blankMode ? <span className="border-b border-slate-400 inline-block w-24">&nbsp;</span> : <strong className="text-teal-700 font-black text-sm">$9,870.00</strong>}</div>
        </div>
      </div>
